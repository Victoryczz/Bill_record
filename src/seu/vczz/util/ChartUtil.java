...
public class ChartUtil {
    ...
    private static String[] sampleLables(List<Record> records){
        String[] sampleLables = new String[records.size()];
    ...
    public static Image getImage(List<Record> rs, int width, int height) {
        ...
        // 根据消费记录得到的下方日期文本
        String[] sampleLabels = sampleLables(rs);
        // 样本中的最大值
    ...
    public static Image getImage(int width, int height) {
        ...
        chart.setFont("legendFont", new Font("Dialog", Font.BOLD, 13));
        // 下方文字的字体
        chart.setFont("sampleLabelFont", new Font("Diolog", Font.BOLD, 13));
        // 图表中间背景颜色
        chart.setChartBackground(Color.white);
        // 图表整体背景颜色
        chart.setChartBackground(ColorUtil.backgroundColor);
        // 把图表转换为Image类型
        Image im = chart.getImage(width, height);
        return im;
    }
